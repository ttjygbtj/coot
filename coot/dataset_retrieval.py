...
import numpy as np
import paddle
# from paddle.io import data as th_data
...
class RetrievalDataset(paddle.io.Dataset):
    ...
    def __getitem__(self, item: int) -> RetrievalDataPointTuple:
        ...
        par_feat = paddle.to_tensor(par_feat).float()
    ...
    def collate_fn(self, data_batch: List[RetrievalDataPointTuple]):
        ...
        vid_feat_len = paddle.to_tensor(list_vid_feat_len).long()
        ...
        par_feat_len = paddle.to_tensor(list_par_feat_len).long()
        ...
        clip_num = paddle.to_tensor(list_clip_num).long()
        ...
        clip_feat_len = paddle.to_tensor(clip_feat_len_list).long()
        ...
        sent_num = paddle.to_tensor(list_sent_num).long()
        ...
        # convert stored lengths to tensor
        sent_feat_len = paddle.to_tensor(sent_feat_len_list).long()
...
def create_retrieval_datasets_and_loaders(cfg: coot.configs_retrieval.RetrievalConfig, path_data: Union[str, Path]) -> (
        Tuple[RetrievalDataset, RetrievalDataset, paddle.io.DataLoader, paddle.io.DataLoader]):
    """
...
def run_retrieval_dataset_test(train_set: RetrievalDataset, train_loader: paddle.io.DataLoader) -> None:
    """
    ...
    for i, batch in enumerate(train_loader):  # type: RetrievalDataBatchTuple
        ...
        for field, value in batch.dict().items():
            print(f"{field}:", end=" ")
            if isinstance(value, paddle.to_tensor):
