...
def cosine_sim(visual_emb: paddle.Tensor, text_emb: paddle.Tensor) -> paddle.to_tensor:
    """
...
class CycleConsistencyLoss(paddle.nn.Layer):
    """
    ...
    def compute_loss_index_gauss(self, emb_mask, _emb_lens, emb_max_len, beta):
        ...
        index_nn = paddle.sum(idx_orig.unsqueeze(1) * beta, dim=-1)
        # shape (batch, seq_len)
        ...
        variance = paddle.sum(distance * beta, dim=-1)
