...
class RetrievalTrainer(trainer_base.BaseTrainer):
    ...
    def compute_align_loss(self, visual_emb: paddle.Tensor, text_emb: paddle.Tensor) -> paddle.to_tensor:
        """
    ...
    def compute_cluster_loss(self, visual_emb: paddle.Tensor, text_emb: paddle.Tensor) -> paddle.to_tensor:
        """
    ...
    def compute_total_constrastive_loss(self, visual_data: model_retrieval.RetrievalVisualEmbTuple,
                                        text_data: model_retrieval.RetrievalTextEmbTuple) -> paddle.to_tensor:
        """
    ...
    def train_model(self, train_loader: paddle.io.DataLoader, val_loader: paddle.io.DataLoader) -> None:
        """
        ...
        for _epoch in range(self.state.current_epoch, self.cfg.train.num_epochs):
            ...
            for step, batch in enumerate(train_loader):  # type: RetrievalDataBatchTuple
                ...
                # ---------- forward pass ----------
                with paddle.amp.auto_cast(enable=self.cfg.fp16_train):
    ...
    @paddle.no_grad()
    def validate_epoch(self, data_loader: paddle.io.DataLoader, val_clips: bool = False, save_embs: bool = False) -> (
        ...
        for _step, batch in enumerate(data_loader):  # type: RetrievalDataBatchTuple
            ...
            with paddle.amp.auto_cast(enable=self.cfg.fp16_val):
                visual_data = self.model_mgr.encode_visual(batch)
                text_data = self.model_mgr.encode_text(batch)
                if self.cfg.train.loss_func == LossesConst.CONTRASTIVE:
                    contr_loss = self.compute_total_constrastive_loss(visual_data, text_data)
        ...
        data_collector_norm = {}
        for key in collect_keys:
            data_collector[key] = paddle.concat(data_collector[key], dim=0).float()
    ...
    def get_opt_state(self) -> Dict[str, Dict[str, paddle.create_parameter]]:
        """
    ...
    def set_opt_state(self, opt_state: Dict[str, Dict[str, paddle.create_parameter]]) -> None:
        """
