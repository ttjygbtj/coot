...
from paddle.utils import data as th_data
...
class MLPModel(paddle.nn.Layer):
    ...
    def forward(self, inputs: paddle.Tensor) -> paddle.to_tensor:
...
class MLPModelManager(models.BaseModelManager):
    ...
    def forward_pass(self, inputs: paddle.Tensor) -> paddle.to_tensor:
        """
        Do forward pass.
...
class MLPMNISTTrainer(trainer_base.BaseTrainer):
    ...
    def compute_ce_loss(self, predictions: paddle.Tensor, labels: paddle.Tensor) -> paddle.to_tensor:
        """
    ...
    def train_model(self, train_loader: th_paddle.io.DataLoader, val_loader: th_paddle.io.DataLoader) -> None:
        """
        Training loop.
    ...
    @paddle.no_grad()
    def validate_epoch(self, data_loader: th_paddle.io.DataLoader) -> (
            Tuple[float, float]):
        """
        ...
        for _step, (inputs, labels) in enumerate(data_loader):
            ...
            acc_total += paddle.sum(th.argmax(predictions, dim=-1) == labels)
