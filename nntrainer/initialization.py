...
def init_weight_(w: paddle.Tensor, init_type="uniform", init_std=1) -> None:
    ...
    if init_type == InitTypesConst.UNIFORM:
        ...
        nn.initializer.XavierUniform(w)
...
def init_network(net: paddle.nn.Layer, init_type: str, init_std: float, verbose: bool = False) -> None:
    """
    Initialize network.
    ...
    for key, val in net.named_parameters():
        ...
        if "genpool_" in key:
            ...
            init_weight_(val.numpy(), init_type, init_std)
            continue
        if "input_rnn." in key:
