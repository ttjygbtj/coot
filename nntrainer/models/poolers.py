...
class MultiGenPool(paddle.nn.Layer):
    """
    ...
    def __init__(
        ...
        self.pools = paddle.nn.LayerList(pools)
    ...
    def forward(self, features, mask, lengths):
        ...
        pooled = paddle.concat(feature_stack, dim=-1)
        return pooled


class GenPool(paddle.nn.Layer):
    """
    ...
    def __init__(
        ...
        x = w1_head
        self.genpool_w1_head = paddle.create_parameter(shape=x.shape,
                                            dtype=str(x.numpy().dtype),
                                            default_initializer=paddle.nn.initializer.Assign(x))
        x = b1_head
        self.genpool_b1_head = paddle.create_parameter(shape=x.shape,
                                            dtype=str(x.numpy().dtype),
                                            default_initializer=paddle.nn.initializer.Assign(x))
        x = w2_head
        self.genpool_w2_head = paddle.create_parameter(shape=x.shape,
                                            dtype=str(x.numpy().dtype),
                                            default_initializer=paddle.nn.initializer.Assign(x))
        x = b2_head
        self.genpool_b2_head = paddle.create_parameter(shape=x.shape,
                                            dtype=str(x.numpy().dtype),
                                            default_initializer=paddle.nn.initializer.Assign(x))
        ...
        self.softmax_temp = 1
        x = paddle.ones([1])
        self.genpool_one = paddle.create_parameter(shape=x.shape,
                                                       dtype=str(x.numpy().dtype),
                                                       default_initializer=paddle.nn.initializer.Assign(x))
        self.genpool_one.stop_gradient = True
...
class TemporalMaxPool(paddle.nn.Layer):
    def forward(self, features, mask, _lengths):
...
class TemporalAvgPool(paddle.nn.Layer):
...
class TemporalAvgPoolFixed(paddle.nn.Layer):
    def forward(self, features, mask, lengths):
        # features (batch, seq_len, feat_dim)
        # mask (batch, seq_len) - 1 for values, 0 for padding
        # lengths (batch)
...
class TemporalLastPool(paddle.nn.Layer):
    """
...
class TemporalFirstPool(paddle.nn.Layer):
    """
