...
class TransformerLegacy(paddle.nn.Layer):
    """
    ...
    def forward(self, features, mask, lengths,
        ...
        if self.use_context:
            ...
            hidden_state = cast(paddle.Tensor, hidden_state.unsqueeze(1))
        ...
        if add_after_pool is not None:
            ...
            pooled = paddle.concat([pooled, add_after_pool], dim=-1)
...
class LearnableClsToken(paddle.nn.Layer):
    """
    ...
    def __init__(self, d_model: int):
        ...
        self.cls_param = paddle.create_parameter(dtype='float32', shape=cls_token)

        # define a single 1 as unlearnable parameter here, so that it will
        # automatically be transfered to GPU
        self.fixed_ones = paddle.create_parameter(dtype='float32', shape=paddle.ones(1), requires_grad=False)
    ...
    def forward(self, features, mask, lengths):
        ...
        features = paddle.concat([self.cls_param.unsqueeze(0).unsqueeze(0).repeat(batch, 1, 1), features], dim=1)
...
class TransformerEncoder(paddle.nn.Layer):
    ...
    def __init__(self, cfg: TransformerEncoderConfig):
        ...
        self.encoder_layers = paddle.nn.LayerList(
...
class Sublayer(paddle.nn.Layer):
    """
    ...
    def forward(self, *args):
        ...
        if isinstance(sublayer_return, paddle.to_tensor):
...
class PointwiseFeedForwardNetwork(paddle.nn.Layer):
    """
