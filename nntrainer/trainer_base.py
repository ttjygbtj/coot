...
from paddle.fluid.clip import set_gradient_clip
...
class BaseTrainer:
    ...
    def __init__(
        ...
        self.logger.info(f"Running on cuda: {self.cfg.use_cuda}, multi-gpu: {self.cfg.use_multi_gpu}, "
                         f"gpus found: {paddle.device.get_device()}, fp16 amp: {self.cfg.fp16_train}.")
    ...
    def hook_post_step(
        ...
        if self.cfg.train.clip_gradient > -1 and not disable_grad_clip:
            ...
            # clip using pytorch
            total_norm = set_gradient_clip(params_flat, self.cfg.train.clip_gradient)
