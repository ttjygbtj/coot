...
# ---------- Multiprocessing ----------
from paddle import concat, reshape, gather

from I_love_paddle.paddle import pd_max
...
def get_truncnorm_tensor(shape: Tuple[int], *, mean: float = 0, std: float = 1, limit: float = 2) -> paddle.to_tensor:
    """
    ...
    num_examples = 8
    tmp = paddle.normal(shape=shape + (num_examples,))
    valid = (tmp < limit).logical_and(tmp > -limit)
    _, ind = pd_max(valid.astype('int'), -1, keepdim=True)
    if len(shape)==2:
        b, c= shape
        t = concat([ reshape(
            concat([reshape(gather(tmp[i, j, :], ind[i, j, :]), [1, -1])
                for j in range(c)], axis=0), [1, c, -1])
                for i in range(b)], axis=0)
    elif len(shape)==1:
        b= shape[0]
        t = concat([reshape(gather(tmp[i,:], ind[i,:]), [1, -1])
                    for i in range(b)], axis=0)
    elif len(shape)==2:
        b, c, d= shape
        t = concat([reshape(
            concat([reshape(
            concat([reshape(gather(tmp[i, j, k, :], ind[i, j, k, :]), [1, -1])
