...
def main():
    ...
    dataloader = paddle.io.DataLoader(dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.workers,
...
class TextDataBatchPoint(TypedNamedTuple):
    ...
    """
    key: List[str]
    tokens: any  # shape: (batch_size, max_num_tokens)
    mask: any  # shape: (batch_size, max_num_tokens)
    lengths: any  # shape: (batch_size)
    sentence_lengths: List[List[int]]


class TextConverterDataset(paddle.io.Dataset):
    """
    ...
    def __getitem__(self, item: int) -> TextDataPointTuple:
        ...
        flat_tokens = paddle.to_tensor([word for sentence in par_tokens for word in sentence]).long()
    ...
    def collate_fn(self, batch: List[TextDataPointTuple]):
        ...
        lengths = paddle.to_tensor(list_lengths).long()

        # initialize batch tensors to the max sequence length
        max_len = max(list_lengths)
        tokens = paddle.zeros(batch_size, max_len).long()
        mask = paddle.zeros(batch_size, max_len).bool()
