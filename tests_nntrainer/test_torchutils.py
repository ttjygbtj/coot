...
def test_set_seed() -> None:
    ...
    """
    set_seed(420)
    rand_int1 = paddle.randint(42, (1,)).squeeze()
...
def test_truncated_normal_fill():
    ...
    numpy.testing.assert_allclose(t.abs() <= 2, np.ones_like(t), err_msg="Tensor out of bounds")
...
def test_get_truncnorm():
    ...
    assert paddle.all(paddle.abs(tn) <= limit * std)
    ...
    for param in net.parameters():
        assert paddle.all(param.data < std * limit)
