...
def test_typednamedtuple() -> None:
    ...
    # correct input
    t = ExampleTuple("key", paddle.zeros(7, 4))

    # correct input with mixed args, kwargs
    ExampleTuple("key", data=paddle.zeros(3, 4))
    ExampleTuple(key="key", data=paddle.zeros(3, 4))
    ...
    with pytest.raises(AssertionError):
        ExampleTuple("key", key="key", data=paddle.zeros(3, 4))

    # too many arguments
    with pytest.raises(AssertionError):
        ExampleTuple("key", paddle.zeros(3, 4), None)
    ...
    # wrong shape
    with pytest.raises(AssertionError):
        ExampleTuple("key", paddle.zeros(3, 6))
    with pytest.raises(AssertionError):
        ExampleTuple("key", paddle.zeros(6))
    with pytest.raises(AssertionError):
        ExampleTuple("key", paddle.zeros(4, 1, 1))
